...
{
	int strlenght = strlen((std::string(e("")) + Class + (char*)e(".") + OffsetName).c_str());
	...
	std::cout << "  " << Class << "." << OffsetName << strspace << e(" ->   0x") << std::hex << std::uppercase << this->FindOffset(Class, OffsetName) << std::endl;
...
DWORD FNTool::FindOffset(const char* Class, const char* varName)
{
	for (DWORD i = 0x0; i < this->ObjectsCount; i++)
	{
		...
		if (!CurrentObject) { itsjustaispoof(MessageBoxA)(0, e("curobjnull"), 0, 0); return NULL; }
		...
		{
			for (auto Property = *(uint64_t*)(CurrentObject + 0x50); !spoof_call(itsjustaispoof(IsBadWritePtr), (LPVOID)Property, (UINT_PTR)8); Property = *(uint64_t*)(Property + 0x20))
			{
				auto Type = *(uint64_t*)(Property + 0x8);

				if (!spoof_call(itsjustaispoof(IsBadWritePtr), (LPVOID)Type, (UINT_PTR)8) && Type)
				{
...
VOID FNTool::FindAndLogClass(const char* Class)
{
	...
	for (DWORD i = 0x0; i < this->ObjectsCount; i++)
	{
		...
		if (!strcmp(CurObjectName, Class)) //Same class
		{
			for (auto Property = *(uint64_t*)(CurrentObject + 0x50); !itsjustaispoof(IsBadWritePtr)((LPVOID)Property, 8); Property = *(uint64_t*)(Property + 0x20))
			{
				auto Type = *(uint64_t*)(Property + 0x8);

				if (!itsjustaispoof(IsBadWritePtr)((LPVOID)Type, 8) && Type)
				{
					auto Property_FName = *(ue::FName*)(Property + 0x28);
					auto Offset = *(DWORD*)(Property + 0x44);
					...
					if (Offset != 0)
					{
						...
						if (Property_idx)
						{
							...
							int strlenght = strlen((std::string(e("")) + PropertyName).c_str());
							int calculation = 52 - strlenght;

							std::string strspace = "";

							for (int iii = 0; iii < calculation; iii++) {
								strspace = strspace + (char*)e(" ");
							}
							int jjjj = 0;
							...
							std::cout << e("  ") << "" << strspace << e(" ->   0x") << std::hex << std::uppercase << Offset << std::endl;
	...
	std::cout << e("  [!] Class doesn't exists\n");
}
