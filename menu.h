...
namespace PYGUI
{
	...
	void Draw_Cursor(bool toogle)
	{
		if (toogle)
		{
			...
			canvas->K2_DrawLine(Vector2{ cursorPos.x, cursorPos.y }, Vector2{ cursorPos.x + 35, cursorPos.y + 10 }, 1, ue::FLinearColor{ 0.21f, 0.20f, 0.21f, 1.00f });
			...
			while (y != 30) //20 steps
			{
				...
				canvas->K2_DrawLine(Vector2{ cursorPos.x, cursorPos.y }, Vector2{ cursorPos.x + x, cursorPos.y + y }, 1, ue::FLinearColor{ 0.21f, 0.20f, 0.21f, 1.00f });
			}

			canvas->K2_DrawLine(Vector2{ cursorPos.x, cursorPos.y }, Vector2{ cursorPos.x + 15, cursorPos.y + 30 }, 1, ue::FLinearColor{ 0.21f, 0.20f, 0.21f, 1.00f });
			canvas->K2_DrawLine(Vector2{ cursorPos.x + 35, cursorPos.y + 10 }, Vector2{ cursorPos.x + 15, cursorPos.y + 30 }, 1, ue::FLinearColor{ 0.21f, 0.20f, 0.21f, 1.00f });
		}
	}
	...
	bool Window(const char* name, Vector2* pos, Vector2 size, bool isOpen)
	{
		...
		//Drop last element
		if (current_element != -1 && !itsjustaispoof(GetAsyncKeyState)(0x1))
		{
			current_element = -1;
		}

		//Drag
		if (hover_element && itsjustaispoof(GetAsyncKeyState)(0x1))
		{
	...
	std::string VirtualKeyCodeToString(UCHAR virtualKey)
	{
		...
		{
			return  (char*)e("MOUSE0");
		}
		if (virtualKey == VK_RBUTTON)
		{
			return  (char*)e("MOUSE1");
		}
		if (virtualKey == VK_MBUTTON)
		{
			return  (char*)e("MBUTTON");
		}
		if (virtualKey == VK_XBUTTON1)
		{
			return  (char*)e("XBUTTON1");
		}
		if (virtualKey == VK_XBUTTON2)
		{
			return (char*)e("XBUTTON2");
		}
	...
	void Hotkey(const char* name, Vector2 size, int* key)
	{
		...
		if (active_hotkey == elements_count)
		{
			//Text
			Vector2 textPos = Vector2{ pos.x + size.x / 2, pos.y + size.y / 2 };
			TextCenter(e("[Press Key]"), textPos, ue::FLinearColor{ 1.0f, 1.0f, 1.0f, 1.0f }, false);

			if (!PYGUI::Input::IsAnyMouseDown())
			{
			...
			if (!already_pressed)
			{
				...
				{
					if (itsjustaispoof(GetAsyncKeyState)(code))
					{
						*key = code;
						active_hotkey = -1;
					}
	...
	void Render()
	{
		for (int i = 0; i < 128; i++)
		{
			if (PostRenderer::drawlist[i].type != -1)
			{
				...
				if (PostRenderer::drawlist[i].type == 1)
				{
					PYGUI::drawFilledRect(PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].size.x, PostRenderer::drawlist[i].size.y, PostRenderer::drawlist[i].color);
				}
				//TextLeft
				else if (PostRenderer::drawlist[i].type == 2)
				{
					PYGUI::TextLeft(PostRenderer::drawlist[i].name, PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].color, PostRenderer::drawlist[i].outline);
				}
				//TextCenter
				else if (PostRenderer::drawlist[i].type == 3)
				{
					PYGUI::TextCenter(PostRenderer::drawlist[i].name, PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].color, PostRenderer::drawlist[i].outline);
				}
				//Draw_Line
...
void jeremycircle(Vector2 pos, int r, ue::FLinearColor outline)
{
