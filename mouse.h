...
namespace MouseController
{
	...
	static BOOLEAN WINAPI Init()
	{
		...
		LPVOID NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(e("win32u")), e("NtUserSendInput"));
		if (!NtUserSendInput_Addr)
		{
			NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(e("user32")), e("NtUserSendInput"));
			if (!NtUserSendInput_Addr)
			{
				// Windows 7 or lower detected
				NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(e("user32")), e("SendInput"));
				if (!NtUserSendInput_Addr)
					return FALSE;
			}
		}
		...
		// windows 8.1 / windows 10
		LPVOID NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(e("win32u")), e("NtUserGetAsyncKeyState"));
		if (!NtUserGetAsyncKeyState_Addr)
		{
			NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(e("user32")), e("NtUserGetAsyncKeyState"));
			if (!NtUserGetAsyncKeyState_Addr)
			{
				// Windows 7 or lower detected
				NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(e("user32")), e("GetAsyncKeyState"));
				if (!NtUserGetAsyncKeyState_Addr)
					return FALSE;
			}
		}
	...
	static BOOLEAN WINAPI click()
	{
		...
		return NtUserSendInput(1, &input1, sizeof(input1));
	}
