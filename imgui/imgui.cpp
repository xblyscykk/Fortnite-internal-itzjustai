...
ImGuiIO::ImGuiIO()
{
	...
	IniSavingRate = 5.0f;
	IniFilename = epysen("1.ini");
	LogFilename = epysen("1.txt");
...
void ImGui::NewFrame()
{
	...
	IM_ASSERT(g.IO.DeltaTime >= 0.0f && epysen("Need a positive DeltaTime (zero is tolerated but will cause some timing issues)"));
	IM_ASSERT(g.IO.DisplaySize.x >= 0.0f && g.IO.DisplaySize.y >= 0.0f && epysen("Invalid DisplaySize value"));
	IM_ASSERT(g.IO.Fonts->Fonts.Size > 0 && epysen("Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?"));
	IM_ASSERT(g.IO.Fonts->Fonts[0]->IsLoaded() && epysen("Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?"));
	IM_ASSERT(g.Style.CurveTessellationTol > 0.0f && epysen("Invalid style setting"));
	IM_ASSERT(g.Style.Alpha >= 0.0f && g.Style.Alpha <= 1.0f && epysen("Invalid style setting. Alpha cannot be negative (allows us to avoid a few clamps in color computations)"));
	IM_ASSERT((g.FrameCount == 0 || g.FrameCountEnded == g.FrameCount) && epysen("Forgot to call Render() or EndFrame() at the end of the previous frame?"));
	for (int n = 0; n < ImGuiKey_COUNT; n++)
		IM_ASSERT(g.IO.KeyMap[n] >= -1 && g.IO.KeyMap[n] < IM_ARRAYSIZE(g.IO.KeysDown) && epysen("io.KeyMap[] contains an out of bound value (need to be 0..512, or -1 for unmapped key)"));
...
static void SettingsHandlerWindow_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
	...
	float x, y;
	int i;
	if (sscanf(line, epysen("Pos=%f,%f"), &x, &y) == 2)         settings->Pos = ImVec2(x, y);
	else if (sscanf(line, epysen("Size=%f,%f"), &x, &y) == 2)   settings->Size = ImMax(ImVec2(x, y), GImGui->Style.WindowMinSize);
	else if (sscanf(line, epysen("Collapsed=%d"), &i) == 1)     settings->Collapsed = (i != 0);
}
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext* imgui_ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
	...
	for (int i = 0; i != g.SettingsWindows.Size; i++)
	{
		...
		if (const char* p = strstr(name, epysen("###")))  // Skip to the "###" marker if any. We don't skip past to match the behavior of GetID()
			name = p;
		buf->appendf(epysen("[%s][%s]\n"), handler->TypeName, name);
		buf->appendf(epysen("Pos=%d,%d\n"), (int)settings->Pos.x, (int)settings->Pos.y);
		buf->appendf(epysen("Size=%d,%d\n"), (int)settings->Size.x, (int)settings->Size.y);
		buf->appendf(epysen("Collapsed=%d\n"), settings->Collapsed);
		buf->appendf(epysen("\n"));
	}
}
...
void ImGui::Initialize()
{
	...
	ini_handler.TypeName = epysen("Window");
	ini_handler.TypeHash = ImHash(epysen("Window"), 0, 0);
...
static void LoadIniSettingsFromDisk(const char* ini_filename)
{
	if (!ini_filename)
		return;
	char* file_data = (char*)ImFileLoadToMemory(ini_filename, epysen("rb"), NULL, +1);
	if (!file_data)
		return;
...
static void SaveIniSettingsToDisk(const char* ini_filename)
{
	...
	FILE* f = ImFileOpen(ini_filename, epysen("wt"));
	if (!f)
		return;
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_render_list, ImDrawList* draw_list)
{
	...
	if (sizeof(ImDrawIdx) == 2)
		IM_ASSERT(draw_list->_VtxCurrentIdx < (1 << 16) && epysen("Too many vertices in ImDrawList using 16-bit indices. Read comment above"));
	...
	for (;;)
	{
		...
		if (line_end != NULL && !(is_last_line && (line_end - text_remaining) == 0))
		{
			...
			if (log_new_line || !is_first_line)
				ImGui::LogText(IM_NEWLINE "%*s%.*s", tree_depth * 4, epysen(""), char_count, text_remaining);
			else
				ImGui::LogText(epysen(" %.*s"), char_count, text_remaining);
		}
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, bool override_previous_tooltip)
{
	ImGuiContext& g = *GImGui;
	char window_name[16];
	ImFormatString(window_name, IM_ARRAYSIZE(window_name), epysen("##Tooltip_%02d"), g.TooltipOverrideCount);
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
	...
	if (extra_flags & ImGuiWindowFlags_ChildMenu)
		ImFormatString(name, IM_ARRAYSIZE(name), epysen("##Menu_%02d"), g.CurrentPopupStack.Size);    // Recycle windows based on depth
	else
		ImFormatString(name, IM_ARRAYSIZE(name), epysen("##Popup_%08x"), id); // Not recycling, so we can close/open during the same frame
...
static bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	...
	char title[256];
	if (name)
		ImFormatString(title, IM_ARRAYSIZE(title), epysen("%s/%s_%08X"), parent_window->Name, name, id);
	else
		ImFormatString(title, IM_ARRAYSIZE(title), epysen("%s/%08X"), parent_window->Name, id);
...
static void CheckStacksSize(ImGuiWindow* window, bool write)
{
	...
	{ int current = window->IDStack.Size;       if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && epysen("PushID/PopID or TreeNode/TreePop Mismatch!"));   p_backup++; }    // Too few or too many PopID()/TreePop()
	{ int current = window->DC.GroupStack.Size; if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && epysen("BeginGroup/EndGroup Mismatch!"));                p_backup++; }    // Too few or too many EndGroup()
	{ int current = g.CurrentPopupStack.Size;   if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && epysen("BeginMenu/EndMenu or BeginPopup/EndPopup Mismatch")); p_backup++; }// Too few or too many EndMenu()/EndPopup()
	{ int current = g.ColorModifiers.Size;      if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && epysen("PushStyleColor/PopStyleColor Mismatch!"));       p_backup++; }    // Too few or too many PopStyleColor()
	{ int current = g.StyleModifiers.Size;      if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && epysen("PushStyleVar/PopStyleVar Mismatch!"));           p_backup++; }    // Too few or too many PopStyleVar()
	{ int current = g.FontStack.Size;           if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && epysen("PushFont/PopFont Mismatch!"));                   p_backup++; }    // Too few or too many PopFont()
	...
	if (first_begin_of_the_frame)
	{
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			...
			if (p_open != NULL)
			{
				...
				if (CloseButton(window->GetID(epysen("#CLOSE")), window->Rect().GetTR() + ImVec2(-PAD - rad, PAD + rad), rad))
					*p_open = false;
			}
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
	...
	const ImGuiStyle& style = g.Style;
	const ImGuiID id = window->GetID(horizontal ? epysen("#SCROLLX") : epysen("#SCROLLY"));
...
const char* ImGui::GetStyleColorName(ImGuiCol idx)
{
	...
	switch (idx)
	{
	case ImGuiCol_Text: return epysen("Text");
	case ImGuiCol_TextDisabled: return epysen("TextDisabled");
	case ImGuiCol_WindowBg: return epysen("WindowBg");
	case ImGuiCol_ChildBg: return epysen("ChildBg");
	case ImGuiCol_PopupBg: return epysen("PopupBg");
	case ImGuiCol_Border: return epysen("Border");
	case ImGuiCol_BorderShadow: return epysen("BorderShadow");
	case ImGuiCol_FrameBg: return epysen("FrameBg");
	case ImGuiCol_FrameBgHovered: return epysen("FrameBgHovered");
	case ImGuiCol_FrameBgActive: return epysen("FrameBgActive");
	case ImGuiCol_TitleBg: return epysen("TitleBg");
	case ImGuiCol_TitleBgActive: return epysen("TitleBgActive");
	case ImGuiCol_TitleBgCollapsed: return epysen("TitleBgCollapsed");
	case ImGuiCol_MenuBarBg: return epysen("MenuBarBg");
	case ImGuiCol_ScrollbarBg: return epysen("ScrollbarBg");
	case ImGuiCol_ScrollbarGrab: return epysen("ScrollbarGrab");
	case ImGuiCol_ScrollbarGrabHovered: return epysen("ScrollbarGrabHovered");
	case ImGuiCol_ScrollbarGrabActive: return epysen("ScrollbarGrabActive");
	case ImGuiCol_CheckMark: return epysen("CheckMark");
	case ImGuiCol_SliderGrab: return epysen("SliderGrab");
	case ImGuiCol_SliderGrabActive: return epysen("SliderGrabActive");
	case ImGuiCol_Button: return epysen("Button");
	case ImGuiCol_ButtonHovered: return epysen("ButtonHovered");
	case ImGuiCol_ButtonActive: return epysen("ButtonActive");
	case ImGuiCol_Header: return epysen("Header");
	case ImGuiCol_HeaderHovered: return epysen("HeaderHovered");
	case ImGuiCol_HeaderActive: return epysen("HeaderActive");
	case ImGuiCol_Separator: return epysen("Separator");
	case ImGuiCol_SeparatorHovered: return epysen("SeparatorHovered");
	case ImGuiCol_SeparatorActive: return epysen("SeparatorActive");
	case ImGuiCol_ResizeGrip: return epysen("ResizeGrip");
	case ImGuiCol_ResizeGripHovered: return epysen("ResizeGripHovered");
	case ImGuiCol_ResizeGripActive: return epysen("ResizeGripActive");
	case ImGuiCol_CloseButton: return epysen("CloseButton");
	case ImGuiCol_CloseButtonHovered: return epysen("CloseButtonHovered");
	case ImGuiCol_CloseButtonActive: return epysen("CloseButtonActive");
	case ImGuiCol_PlotLines: return epysen("PlotLines");
	case ImGuiCol_PlotLinesHovered: return epysen("PlotLinesHovered");
	case ImGuiCol_PlotHistogram: return epysen("PlotHistogram");
	case ImGuiCol_PlotHistogramHovered: return epysen("PlotHistogramHovered");
	case ImGuiCol_TextSelectedBg: return epysen("TextSelectedBg");
	case ImGuiCol_ModalWindowDarkening: return epysen("ModalWindowDarkening");
	case ImGuiCol_DragDropTarget: return epysen("DragDropTarget");
	}
	IM_ASSERT(0);
	return epysen("Unknown");
}
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
	...
	const ImGuiID id = window->GetID(epysen("#image"));
	PopID();
...
void ImGui::LogToFile(int max_depth, const char* filename)
{
	...
	g.LogFile = ImFileOpen(filename, epysen("ab"));
	if (!g.LogFile)
	{
...
void ImGui::LogButtons()
{
	...
	PushID(epysen("LogButtons"));
	const bool log_to_tty = Button(epysen("Log To TTY")); SameLine();
	const bool log_to_file = Button(epysen("Log To File")); SameLine();
	const bool log_to_clipboard = Button(epysen("Log To Clipboard")); SameLine();
	PushItemWidth(80.0f);
	PushAllowKeyboardFocus(false);
	SliderInt(epysen("Depth"), &g.LogAutoExpandMaxDepth, 0, 9, NULL);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
	...
	else
	{
		...
		if (g.LogEnabled)
			LogRenderedText(&text_pos, epysen(">"));
...
static inline void DataTypeFormatString(ImGuiDataType data_type, void* data_ptr, int decimal_precision, char* buf, int buf_size)
{
	if (data_type == ImGuiDataType_Int)
	{
		if (decimal_precision < 0)
			ImFormatString(buf, buf_size, epysen("%d"), *(int*)data_ptr);
		else
			ImFormatString(buf, buf_size, epysen("%.*d"), decimal_precision, *(int*)data_ptr);
	}
	else if (data_type == ImGuiDataType_Float)
	{
		if (decimal_precision < 0)
			ImFormatString(buf, buf_size, epysen("%f"), *(float*)data_ptr);     // Ideally we'd have a minimum decimal precision of 1 to visually denote that it is a float, while hiding non-significant digits?
		else
			ImFormatString(buf, buf_size, epysen("%.*f"), decimal_precision, *(float*)data_ptr);
	}
}
...
static bool DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* scalar_format)
{
	...
	if (data_type == ImGuiDataType_Int)
	{
		...
		float arg1f = 0.0f;
		if (op == '+') { if (sscanf(buf, epysen("%f"), &arg1f) == 1) *v = (int)(arg0i + arg1f); }                 // Add (use "+-" to subtract)
		else if (op == '*') { if (sscanf(buf, epysen("%f"), &arg1f) == 1) *v = (int)(arg0i * arg1f); }                 // Multiply
		else if (op == '/') { if (sscanf(buf, epysen("%f"), &arg1f) == 1 && arg1f != 0.0f) *v = (int)(arg0i / arg1f); }// Divide
...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max)
{
	...
	bool value_changed = SliderFloat(label, &v_deg, v_degrees_min, v_degrees_max, epysen("%.0f deg"), 1.0f);
...
bool ImGui::SliderFloatN(const char* label, float* v, int components, float v_min, float v_max, const char* display_format, float power)
{
	...
	{
		PushID(i);
		value_changed |= SliderFloat(epysen("##v"), &v[i], v_min, v_max, display_format, power);
...
bool ImGui::SliderIntN(const char* label, int* v, int components, int v_min, int v_max, const char* display_format)
{
	...
	{
		PushID(i);
		value_changed |= SliderInt(epysen("##v"), &v[i], v_min, v_max, display_format);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::DragFloatN(const char* label, float* v, int components, float v_speed, float v_min, float v_max, const char* display_format, float power)
{
	...
	{
		PushID(i);
		value_changed |= DragFloat(epysen("##v"), &v[i], v_speed, v_min, v_max, display_format, power);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* display_format, const char* display_format_max, float power)
{
	...
	bool value_changed = DragFloat(epysen("##min"), v_current_min, v_speed, (v_min >= v_max) ? -FLT_MAX : v_min, (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max), display_format, power);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
	value_changed |= DragFloat(epysen("##max"), v_current_max, v_speed, (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min), (v_min >= v_max) ? FLT_MAX : v_max, display_format_max ? display_format_max : display_format, power);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragIntN(const char* label, int* v, int components, float v_speed, int v_min, int v_max, const char* display_format)
{
	...
	{
		PushID(i);
		value_changed |= DragInt(epysen("##v"), &v[i], v_speed, v_min, v_max, display_format);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* display_format, const char* display_format_max)
{
	...
	bool value_changed = DragInt(epysen("##min"), v_current_min, v_speed, (v_min >= v_max) ? INT_MIN : v_min, (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max), display_format);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
	value_changed |= DragInt(epysen("##max"), v_current_max, v_speed, (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min), (v_min >= v_max) ? INT_MAX : v_max, display_format_max ? display_format_max : display_format);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
	...
	if (values_count > 0)
	{
		...
		if (hovered)
		{
			...
			if (plot_type == ImGuiPlotType_Lines)
				SetTooltip(epysen("%d: %8.4g\n%d: %8.4g"), v_idx, v0, v_idx + 1, v1);
			else if (plot_type == ImGuiPlotType_Histogram)
				SetTooltip(epysen("%d: %8.4g"), v_idx, v0);
			v_hovered = v_idx;
		}
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
	...
	if (!overlay)
	{
		ImFormatString(overlay_buf, IM_ARRAYSIZE(overlay_buf), epysen("%.0f%%"), fraction * 100 + 0.01f);
		overlay = overlay_buf;
	}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (g.LogEnabled)
		LogRenderedText(&text_bb.Min, *v ? epysen("[x]") : epysen("[ ]"));
	if (label_size.x > 0.0f)
...
bool ImGui::RadioButton(const char* label, bool active)
{
	...
	if (g.LogEnabled)
		LogRenderedText(&text_bb.Min, active ? epysen("(x)") : epysen("( )"));
	if (label_size.x > 0.0f)
		RenderText(text_bb.Min, label);
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	const bool is_currently_scrolling = (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetIDNoKeepAlive(epysen("#SCROLLY")));
...
bool ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags)
{
	...
	if (step_ptr)
	{
		PopItemWidth();
		SameLine(0, style.ItemInnerSpacing.x);
		if (ButtonEx(epysen("-"), button_sz, ImGuiButtonFlags_Repeat | ImGuiButtonFlags_DontClosePopups))
		{
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
	...
	if (decimal_precision < 0)
		strcpy(display_format, epysen("%f"));      // Ideally we'd have a minimum decimal precision of 1 to visually denote that this is a float, while hiding non-significant digits? %f doesn't have a minimum of 1
	else
		ImFormatString(display_format, IM_ARRAYSIZE(display_format), epysen("%%.%df"), decimal_precision);
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags extra_flags)
{
	...
	const char* scalar_format = (extra_flags & ImGuiInputTextFlags_CharsHexadecimal) ? epysen("%08X") : epysen("%d");
...
bool ImGui::InputFloatN(const char* label, float* v, int components, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
	...
	{
		PushID(i);
		value_changed |= InputFloat(epysen("##v"), &v[i], 0, 0, decimal_precision, extra_flags);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::InputIntN(const char* label, int* v, int components, ImGuiInputTextFlags extra_flags)
{
	...
	{
		PushID(i);
		value_changed |= InputInt(epysen("##v"), &v[i], 0, 0, extra_flags);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	char name[16];
	ImFormatString(name, IM_ARRAYSIZE(name), epysen("##Combo_%02d"), g.CurrentPopupStack.Size); // Recycle windows based on depth
...
bool ImGui::BeginMainMenuBar()
{
	...
	if (!Begin(epysen("##MainMenuBar"), NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_MenuBar)
		|| !BeginMenuBar())
	{
		End();
		PopStyleVar(2);
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
	...
	ColorButton(epysen("##preview"), ImVec4(col[0], col[1], col[2], col[3]), (flags & (ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip, sz);
	SameLine();
	if (flags & ImGuiColorEditFlags_NoAlpha)
		Text(epysen("#%02X%02X%02X\nR: %d, G: %d, B: %d\n(%.3f, %.3f, %.3f)"), cr, cg, cb, cr, cg, cb, col[0], col[1], col[2]);
	else
		Text(epysen("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)"), cr, cg, cb, ca, cr, cg, cb, ca, col[0], col[1], col[2], col[3]);
	EndTooltip();
}
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
	...
	if ((!allow_opt_inputs && !allow_opt_datatype) || !BeginPopup(epysen("context")))
		return;
	ImGuiContext& g = *GImGui;
	ImGuiColorEditFlags opts = g.ColorEditOptions;
	if (allow_opt_inputs)
	{
		if (RadioButton(epysen("RGB"), (opts & ImGuiColorEditFlags_RGB) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_RGB;
		if (RadioButton(epysen("HSV"), (opts & ImGuiColorEditFlags_HSV) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_HSV;
		if (RadioButton(epysen("HEX"), (opts & ImGuiColorEditFlags_HEX) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_HEX;
	}
	if (allow_opt_datatype)
	{
		if (allow_opt_inputs) Separator();
		if (RadioButton(epysen("0..255"), (opts & ImGuiColorEditFlags_Uint8) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) | ImGuiColorEditFlags_Uint8;
		if (RadioButton(epysen("0.00..1.00"), (opts & ImGuiColorEditFlags_Float) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) | ImGuiColorEditFlags_Float;
	}

	if (allow_opt_inputs || allow_opt_datatype)
		Separator();
	if (Button(epysen("Copy as.."), ImVec2(-1, 0)))
		OpenPopup(epysen("Copy"));
	if (BeginPopup(epysen("Copy")))
	{
		int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]), cb = IM_F32_TO_INT8_SAT(col[2]), ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
		char buf[64];
		ImFormatString(buf, IM_ARRAYSIZE(buf), epysen("(%.3ff, %.3ff, %.3ff, %.3ff)"), col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
		if (Selectable(buf))
			SetClipboardText(buf);
		ImFormatString(buf, IM_ARRAYSIZE(buf), epysen("(%d,%d,%d,%d)"), cr, cg, cb, ca);
		if (Selectable(buf))
			SetClipboardText(buf);
		if (flags & ImGuiColorEditFlags_NoAlpha)
			ImFormatString(buf, IM_ARRAYSIZE(buf), epysen("0x%02X%02X%02X"), cr, cg, cb);
		else
			ImFormatString(buf, IM_ARRAYSIZE(buf), epysen("0x%02X%02X%02X%02X"), cr, cg, cb, ca);
		if (Selectable(buf))
			SetClipboardText(buf);
		EndPopup();
	}
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if ((!allow_opt_picker && !allow_opt_alpha_bar) || !ImGui::BeginPopup(epysen("context")))
		return;
	ImGuiContext& g = *GImGui;
	if (allow_opt_picker)
	{
		...
		for (int picker_type = 0; picker_type < 2; picker_type++)
		{
			...
			if (ImGui::Selectable(epysen("##selectable"), false, 0, picker_size)) // By default, Selectable() is closing popup
				g.ColorEditOptions = (g.ColorEditOptions & ~ImGuiColorEditFlags__PickerMask) | (picker_flags & ImGuiColorEditFlags__PickerMask);
			ImGui::SetCursorScreenPos(backup_pos);
			ImVec4 dummy_ref_col;
			memcpy(&dummy_ref_col.x, ref_col, sizeof(float) * (picker_flags & ImGuiColorEditFlags_NoAlpha ? 3 : 4));
			ImGui::ColorPicker4(epysen("##dummypicker"), &dummy_ref_col.x, picker_flags);
			ImGui::PopID();
		}
		ImGui::PopItemWidth();
	}
	if (allow_opt_alpha_bar)
	{
		...
		ImGui::CheckboxFlags(epysen("Alpha Bar"), (unsigned int*)&g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
	}
	ImGui::EndPopup();
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		const bool hide_prefix = (w_item_one <= CalcTextSize((flags & ImGuiColorEditFlags_Float) ? epysen("M:0.000") : epysen("M:000")).x);
		...
		for (int n = 0; n < components; n++)
		{
			...
			if (!(flags & ImGuiColorEditFlags_NoOptions))
				OpenPopupOnItemClick(epysen("context"));
		}
		PopItemWidth();
		PopItemWidth();
	}
	else if ((flags & ImGuiColorEditFlags_HEX) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		char buf[64];
		if (alpha)
			ImFormatString(buf, IM_ARRAYSIZE(buf), epysen("#%02X%02X%02X%02X"), ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255), ImClamp(i[3], 0, 255));
		else
			ImFormatString(buf, IM_ARRAYSIZE(buf), epysen("#%02X%02X%02X"), ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255));
		PushItemWidth(w_items_all);
		if (InputText(epysen("##Text"), buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
		{
			value_changed = true;
			char* p = buf;
			...
			i[0] = i[1] = i[2] = i[3] = 0;
			if (alpha)
				sscanf(p, epysen("%02X%02X%02X%02X"), (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2], (unsigned int*)&i[3]); // Treat at unsigned (%X is unsigned)
			else
				sscanf(p, epysen("%02X%02X%02X"), (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2]);
		}
		if (!(flags & ImGuiColorEditFlags_NoOptions))
			OpenPopupOnItemClick(epysen("context"));
		PopItemWidth();
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
	{
		...
		if (ColorButton(epysen("##ColorButton"), col_v4, flags))
		{
			if (!(flags & ImGuiColorEditFlags_NoPicker))
			{
				// Store current color and open a picker
				g.ColorPickerRef = col_v4;
				OpenPopup(epysen("picker"));
		...
		if (BeginPopup(epysen("picker")))
		{
			...
			value_changed |= ColorPicker4(epysen("##picker"), col, picker_flags, &g.ColorPickerRef.x);
			PopItemWidth();
			EndPopup();
		}
	}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel)
	{
		...
		InvisibleButton(epysen("hsv"), ImVec2(sv_picker_size + style.ItemInnerSpacing.x + bars_width, sv_picker_size));
		if (IsItemActive())
		{
	...
	{
		// SV rectangle logic
		InvisibleButton(epysen("sv"), ImVec2(sv_picker_size, sv_picker_size));
		if (IsItemActive())
		{
		...
		InvisibleButton(epysen("hue"), ImVec2(bars_width, sv_picker_size));
		if (IsItemActive())
		{
	...
	if (alpha_bar)
	{
		...
		InvisibleButton(epysen("alpha"), ImVec2(bars_width, sv_picker_size));
		if (IsItemActive())
		{
	...
	if (!(flags & ImGuiColorEditFlags_NoSidePreview))
	{
		...
		if ((flags & ImGuiColorEditFlags_NoLabel))
			Text(epysen("Current"));
		ColorButton(epysen("##current"), col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)), ImVec2(square_sz * 3, square_sz * 2));
		if (ref_col != NULL)
		{
			Text(epysen("Original"));
			ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
			if (ColorButton(epysen("##original"), ref_col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)), ImVec2(square_sz * 3, square_sz * 2)))
			{
	...
	if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		if (flags & ImGuiColorEditFlags_RGB || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4(epysen("##rgb"), col, sub_flags | ImGuiColorEditFlags_RGB);
		if (flags & ImGuiColorEditFlags_HSV || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4(epysen("##hsv"), col, sub_flags | ImGuiColorEditFlags_HSV);
		if (flags & ImGuiColorEditFlags_HEX || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4(epysen("##hex"), col, sub_flags | ImGuiColorEditFlags_HEX);
		PopItemWidth();
	}
...
void ImGui::Separator(float heightdistance)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	const ImRect bb(ImVec2(x1, window->DC.CursorPos.y - heightdistance), ImVec2(x2, window->DC.CursorPos.y + 1.0f - heightdistance));
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
	...
	ImGuiID id = window->GetID(str_id ? str_id : epysen("columns"));
	PopID();
...
void ImGui::TreePush(const char* str_id)
{
	...
	Indent();
	window->DC.TreeDepth++;
	PushID(str_id ? str_id : epysen("#TreePush"));
}
...
void ImGui::TreePush(const void* ptr_id)
{
	...
	Indent();
	window->DC.TreeDepth++;
	PushID(ptr_id ? ptr_id : (const void*)epysen("#TreePush"));
}
...
{
	Text(epysen("%s: %s"), prefix, (b ? epysen("true") : epysen("false")));
}

void ImGui::Value(const char* prefix, int v)
{
	Text(epysen("%s: %d"), prefix, v);
}
...
void ImGui::Value(const char* prefix, float v, const char* float_format)
{
	...
	{
		char fmt[64];
		ImFormatString(fmt, IM_ARRAYSIZE(fmt), epysen("%%s: %s"), float_format);
		Text(fmt, prefix, v);
	}
	else
	{
		Text(epysen("%s: %.3f"), prefix, v);
	}
}
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags, int mouse_button)
{
	...
	{
		window = NULL;
		source_id = ImHash(epysen("#SourceExtern"), 0);
...
bool ImGui::SetDragDropPayload(const char* type, const void* data, size_t data_size, ImGuiCond cond)
{
	...
	IM_ASSERT(type != NULL);
	IM_ASSERT(strlen(type) < IM_ARRAYSIZE(payload.DataType) && epysen("Payload type can be at most 12 characters long"));
...
{
	if (ImGui::Begin(epysen("ImGui Metrics"), p_open))
	{
		ImGui::Text(epysen("Dear ImGui %s"), ImGui::GetVersion());
		ImGui::Text(epysen("Application average %.3f ms/frame (%.1f FPS)"), 1000.0f / ImGui::GetIO().Framerate, ImGui::GetIO().Framerate);
		ImGui::Text(epysen("%d vertices, %d indices (%d triangles)"), ImGui::GetIO().MetricsRenderVertices, ImGui::GetIO().MetricsRenderIndices, ImGui::GetIO().MetricsRenderIndices / 3);
		ImGui::Text(epysen("%d allocations"), ImGui::GetIO().MetricsAllocs);
		static bool show_clip_rects = true;
		ImGui::Checkbox(epysen("Show clipping rectangles when hovering an ImDrawCmd"), &show_clip_rects);
		ImGui::Separator();
		...
		struct Funcs
		{
			...
			{
				bool node_open = ImGui::TreeNode(draw_list, epysen("%s: '%s' %d vtx, %d indices, %d cmds"), label, draw_list->_OwnerName ? draw_list->_OwnerName : "", draw_list->VtxBuffer.Size, draw_list->IdxBuffer.Size, draw_list->CmdBuffer.Size);
				if (draw_list == ImGui::GetWindowDrawList())
				{
					ImGui::SameLine();
					ImGui::TextColored(ImColor(255, 100, 100), epysen("CURRENTLY APPENDING")); // Can't display stats for active draw list! (we don't have the data double-buffered)
				...
				for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
				{
					...
					if (pcmd->UserCallback)
					{
						ImGui::BulletText(epysen("Callback %p, user_data %p"), pcmd->UserCallback, pcmd->UserCallbackData);
						continue;
					}
					ImDrawIdx* idx_buffer = (draw_list->IdxBuffer.Size > 0) ? draw_list->IdxBuffer.Data : NULL;
					bool pcmd_node_open = ImGui::TreeNode((void*)(pcmd - draw_list->CmdBuffer.begin()), epysen("Draw %-4d %s vtx, tex = %p, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)"), pcmd->ElemCount, draw_list->IdxBuffer.Size > 0 ? "indexed" : "non-indexed", pcmd->TextureId, pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w);
					...
					while (clipper.Step())
						for (int prim = clipper.DisplayStart, vtx_i = elem_offset + clipper.DisplayStart * 3; prim < clipper.DisplayEnd; prim++)
						{
							...
							for (int n = 0; n < 3; n++, vtx_i++)
							{
								...
								buf_p += ImFormatString(buf_p, (int)(buf_end - buf_p), epysen("%s %04d { pos = (%8.2f,%8.2f), uv = (%.6f,%.6f), col = %08X }\n"), (n == 0) ? epysen("vtx") : epysen("   "), vtx_i, v.pos.x, v.pos.y, v.uv.x, v.uv.y, v.col);
							}
			...
			{
				if (!ImGui::TreeNode(label, epysen("%s (%d)"), label, windows.Size))
					return;
				for (int i = 0; i < windows.Size; i++)
					Funcs::NodeWindow(windows[i], epysen("Window"));
				ImGui::TreePop();
			}

			static void NodeWindow(ImGuiWindow* window, const char* label)
			{
				if (!ImGui::TreeNode(window, epysen("%s '%s', %d @ 0x%p"), label, window->Name, window->Active || window->WasActive, window))
					return;
				NodeDrawList(window, window->DrawList, epysen("DrawList"));
				ImGui::BulletText(epysen("Pos: (%.1f,%.1f), Size: (%.1f,%.1f), SizeContents (%.1f,%.1f)"), window->Pos.x, window->Pos.y, window->Size.x, window->Size.y, window->SizeContents.x, window->SizeContents.y);
				ImGui::BulletText(epysen("Scroll: (%.2f/%.2f,%.2f/%.2f)"), window->Scroll.x, GetScrollMaxX(window), window->Scroll.y, GetScrollMaxY(window));
				ImGui::BulletText(epysen("Active: %d, WriteAccessed: %d"), window->Active, window->WriteAccessed);
				if (window->RootWindow != window) NodeWindow(window->RootWindow, epysen("RootWindow"));
				if (window->DC.ChildWindows.Size > 0) NodeWindows(window->DC.ChildWindows, epysen("ChildWindows"));
				ImGui::BulletText(epysen("Storage: %d bytes"), window->StateStorage.Data.Size * (int)sizeof(ImGuiStorage::Pair));
				ImGui::TreePop();
			}
		};
		...
		ImGuiContext& g = *GImGui;
		Funcs::NodeWindows(g.Windows, epysen("Windows"));
		if (ImGui::TreeNode(epysen("DrawList"), epysen("Active DrawLists (%d)"), g.DrawDataBuilder.Layers[0].Size))
		{
			for (int i = 0; i < g.DrawDataBuilder.Layers[0].Size; i++)
				Funcs::NodeDrawList(NULL, g.DrawDataBuilder.Layers[0][i], epysen("DrawList"));
			ImGui::TreePop();
		}
		if (ImGui::TreeNode(epysen("Popups"), epysen("Open Popups Stack (%d)"), g.OpenPopupStack.Size))
		{
			for (int i = 0; i < g.OpenPopupStack.Size; i++)
			{
				ImGuiWindow* window = g.OpenPopupStack[i].Window;
				ImGui::BulletText(epysen("PopupID: %08x, Window: '%s'%s%s"), g.OpenPopupStack[i].PopupId, window ? window->Name : epysen("NULL"), window && (window->Flags & ImGuiWindowFlags_ChildWindow) ? epysen(" ChildWindow") : epysen(""), window && (window->Flags & ImGuiWindowFlags_ChildMenu) ? epysen(" ChildMenu") : epysen(""));
			}
			ImGui::TreePop();
		}
		if (ImGui::TreeNode(epysen("Basic state")))
		{
			ImGui::Text(epysen("HoveredWindow: '%s'"), g.HoveredWindow ? g.HoveredWindow->Name : epysen("NULL"));
			ImGui::Text(epysen("HoveredRootWindow: '%s'"), g.HoveredRootWindow ? g.HoveredRootWindow->Name : epysen("NULL"));
			ImGui::Text(epysen("HoveredId: 0x%08X/0x%08X (%.2f sec)"), g.HoveredId, g.HoveredIdPreviousFrame, g.HoveredIdTimer); // Data is "in-flight" so depending on when the Metrics window is called we may see current frame information or not
			ImGui::Text(epysen("ActiveId: 0x%08X/0x%08X (%.2f sec)"), g.ActiveId, g.ActiveIdPreviousFrame, g.ActiveIdTimer);
			ImGui::Text(epysen("ActiveIdWindow: '%s'"), g.ActiveIdWindow ? g.ActiveIdWindow->Name : epysen("NULL"));
			ImGui::Text(epysen("NavWindow: '%s'"), g.NavWindow ? g.NavWindow->Name : epysen("NULL"));
			ImGui::Text(epysen("DragDrop: %d, SourceId = 0x%08X, Payload \"%s\" (%d bytes)"), g.DragDropActive, g.DragDropPayload.SourceId, g.DragDropPayload.DataType, g.DragDropPayload.DataSize);
			ImGui::TreePop();
		}
	}
	ImGui::End();
